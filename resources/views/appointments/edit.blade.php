<x-layout>
    ...
    <h2>Edit appointment <strong>{{ $appointment->id }}</strong></h2>
    ...
    <form method="POST" action="{{ route("appointments.update", ['appointment' => $appointment->id]) }}">
        ...
        <p>
            ...
            <input
                ...
                value="{{ old('timestamp') ?? $appointment->timestamp }}"
            />
        @error('timestamp')
        ...
        <p>
            ...
            <input
                ...
                value="{{ old('names') ?? $appointment->names }}"
            />
        @error('names')
        <div class="alert alert-danger">{{ $message }}</div>
        @enderror
        ...
        <p>
            ...
            >
                @foreach($notificationTypes as $nt)
                    <option value="{{$nt->id}}"
                        {{(old('notification_type_id') ?? $appointment->notification_type_id) == $nt->id ? 'selected' : ''}}
                    >{{$nt->name}}</option>
                @endforeach
            </select>
